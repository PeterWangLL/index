...
function buildItems(pool: ImageItem[], seg: number): ItemDef[] {
  ...
  const remainder = totalSlots % normalizedImages.length;

  let imagePool: { src: string; alt: string }[] = [];
  for (let i = 0; i < normalizedImages.length; i++) {
    const count = baseRepeats + (i < remainder ? 1 : 0);
    for (let j = 0; j < count; j++) {
      imagePool.push({ ...normalizedImages[i] });
    }
  }
  ...
  // 相邻去重兜底
  for (let i = 1; i < imagePool.length; i++) {
    if (imagePool[i].src === imagePool[i - 1].src) {
      for (let j = i + 1; j < imagePool.length; j++) {
        if (imagePool[j].src !== imagePool[i].src) {
          const tmp = imagePool[i];
