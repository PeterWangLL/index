...
| 技术 | 版本/说明 | 选型理由 |
|------|-----------|----------|
| **Next.js** (App Router) | 16+ | React 生态中最成熟的元框架，支持 Static Export，完美适配 GitHub Pages 部署。 |
...
| 技术 | 说明 |
|------|------|
| **gray-matter** | 解析 Markdown 文件的 YAML frontmatter（标题、日期、标签等）。 |
| **react-markdown** | 将 Markdown 字符串渲染为 React 组件，避免使用 `dangerouslySetInnerHTML`。 |
| **remark-gfm** | 支持 GitHub Flavored Markdown（表格、任务列表、删除线等）。 |
| **rehype-slug** | 为文章标题自动注入 `id` 锚点。 |
| **rehype-autolink-headings** | 为标题锚点添加可点击链接，便于分享和导航。 |
...
│   └── images/
├── spec/                # 规格文档与历史记录
└── next.config.ts       # Static Export 配置
```
...
- `basePath`：因最终绑定自定义域名 `www.peterwangll.top`，无需设置 basePath。
